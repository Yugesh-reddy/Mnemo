"""Execute each predeclared v10 phase exactly once (all local; no paid calls).

Phases:
  luna-legacy / luna-lasting  replay the frozen v8 Luna candidates, legacy vs lasting tiering
...
def score(manifest: dict) -> int:
    ...
    names = ("luna-legacy-score", "luna-lasting-score", "qwen-lasting-score", "decision")
    ...
    reports, scores = {}, {}
    for arm in ("luna-legacy", "luna-lasting", "qwen-lasting"):
    ...
    labels = json.loads((HERE / "durability-labels.json").read_text())["rows"]
    tiers_by_arm = {arm: candidate_tiers(reports[arm]) for arm in ("luna-legacy", "luna-lasting")}
    label_table = {
...
def main() -> None:
    phases = ("luna-legacy", "luna-lasting", "qwen-lasting", "eval", "score")
    parser = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    parser.add_argument("phase", choices=phases)
    args = parser.parse_args()
    manifest = json.loads(MANIFEST.read_text())
    preflight(manifest)
    if args.phase in ("luna-legacy", "luna-lasting"):
