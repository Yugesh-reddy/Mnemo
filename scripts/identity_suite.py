...
async def run_case(
    case: dict[str, Any], *, embedder: Any, verifier: Any, settings: Settings, dsn: str
) -> dict[str, Any]:
    ...
    try:
        ...
        worker = ExtractionWorker(
            conn, embedder, AuthoredExtractor(case["turns"]), verifier, settings=settings
        )
...
async def run_suite(
    ...
    *,
    ...
    results = [
        await run_case(case, embedder=embedder, verifier=verifier, settings=settings, dsn=dsn)
        for case in cases
    ]
    return {
